...
const getEvents = authActionClient.metadata({
    roleGate: "USER",
}).action(async () => {
    return await eventService.getEvents();
});

const getEventById = authActionClient.metadata({
    roleGate: "USER",
}).schema(z.object({
    id: z.string(),
})).action(async (data) => {
...
const createEvent = authActionClient.metadata({
    roleGate: "ADMIN",
}).schema(createEventSchema).action(async (data) => {
    return await eventService.createEvent(data.parsedInput);
});

const updateEvent = authActionClient.metadata({
    roleGate: "ADMIN",
}).schema(updateEventSchema).action(async (data) => {
    return await eventService.updateEvent(data.parsedInput.id, data.parsedInput);
});

const deleteEvent = authActionClient.metadata({
    roleGate: "ADMIN",
}).schema(z.object({
    id: z.string(),
})).action(async (data) => {
